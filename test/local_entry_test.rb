...
class ZipLocalEntryTest < MiniTest::Unit::TestCase
  ...
  def test_read_local_entryHeaderOfFirstTestZipEntry
    ::File.open(TestZipFile::TEST_ZIP3.zip_name, "rb") do |file|
      entry = ::RubyZip::Entry.read_local_entry(file)
      ...
      assert_equal(::RubyZip::Entry::DEFLATED, entry.compression_method)
  ...
  def test_readDateTime
    ::File.open("test/data/rubycode.zip", "rb") {
      ...
      assert_equal(::RubyZip::DOSTime.at(1019261638), entry.time)
    }
  end
  ...
  def test_read_local_entryFromNonZipFile
    ::File.open("test/data/file2.txt") {
        |file|
      assert_equal(nil, ::RubyZip::Entry.read_local_entry(file))
    }
  end
  ...
  def test_writeEntry
    entry = ::RubyZip::Entry.new("file.zip", "entryName", "my little comment",
                             "thisIsSomeExtraInformation", 100, 987654,
                             ::RubyZip::Entry::DEFLATED, 400)
  ...
  def test_write64Entry
    ::RubyZip.write_zip64_support = true
    entry = ::RubyZip::Entry.new("bigfile.zip", "entryName", "my little equine",
                             "malformed extra field because why not",
                             0x7766554433221100, 0xDEADBEEF, ::RubyZip::Entry::DEFLATED,
                             0x9988776655443322)
    write_to_file("localEntryHeader.bin", "centralEntryHeader.bin", entry)
    entryReadLocal, entryReadCentral = read_from_file("localEntryHeader.bin", "centralEntryHeader.bin")
    compare_local_entry_headers(entry, entryReadLocal)
  ...
  def test_rewriteLocalHeader64
    ::RubyZip.write_zip64_support = true
    buf1 = StringIO.new
    entry = ::RubyZip::Entry.new("file.zip", "entryName")
  ...
  def test_readLocalOffset
    ...
    read_entry = nil
    ::File.open('centralEntryHeader.bin', 'rb') { |f| read_entry = ::RubyZip::Entry.read_c_dir_entry(f) }
  ...
  def read_from_file(localFileName, centralFileName)
    localEntry = nil
    cdirEntry = nil
    ::File.open(localFileName, "rb") { |f| localEntry = ::RubyZip::Entry.read_local_entry(f) }
    ::File.open(centralFileName, "rb") { |f| cdirEntry = ::RubyZip::Entry.read_c_dir_entry(f) }
