module RubyZip
  ...
  #   require 'zip'
  #
  #   RubyZip::File.open("my.zip", RubyZip::File::CREATE) {
  #    |zipfile|
  ...
  class File < CentralDirectory
    ...
    class << self
      ...
      def open(file_name, create = nil)
        zf = ::RubyZip::File.new(file_name, create)
      ...
      def add_buffer
        io = ::StringIO.new('')
        zf = ::RubyZip::File.new(io, true, true)
        yield zf
        zf.write_buffer(io)
      end
      ...
      def open_buffer(io, options = {})
        unless io.is_a?(IO) || io.is_a?(String) || io.is_a?(Tempfile)
          raise "RubyZip::File.open_buffer expects an argument of class String, IO, or Tempfile. Found: #{io.class}"
        end
        if io.is_a?(::String)
          require 'stringio'
          io = ::StringIO.new(io)
        end
        zf = ::RubyZip::File.new(io, true, true, options)
    ...
    # of RubyZip::Entry, a new RubyZip::Entry will be initialized using the arguments
    ...
    def add(entry, src_path, &continue_on_exists_proc)
      continue_on_exists_proc ||= proc { ::RubyZip.continue_on_exists_proc }
      check_entry_exists(entry, continue_on_exists_proc, "add")
      new_entry = entry.kind_of?(::RubyZip::Entry) ? entry : ::RubyZip::Entry.new(@name, entry.to_s)
    ...
    def commit
      ...
      on_success_replace do |tmp_file|
        ::RubyZip::OutputStream.open(tmp_file) do |zos|
          @entry_set.each do |e|
    ...
    def write_buffer(io = ::StringIO.new(''))
      ::RubyZip::OutputStream.write_buffer(io) do |zos|
    ...
    def commit_required?
      ...
      @comment != @stored_comment || @entry_set != @stored_entries || @create == ::RubyZip::File::CREATE
    end
    ...
    def mkdir(entryName, permissionInt = 0755)
      ...
      @entry_set << ::RubyZip::StreamableDirectory.new(@name, entryName, nil, permissionInt)
    end
