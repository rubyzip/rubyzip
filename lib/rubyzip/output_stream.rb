module RubyZip
  ...
  class OutputStream
    ...
    def initialize(file_name, stream=false)
      ...
      @entry_set = ::RubyZip::EntrySet.new
      @compressor = ::RubyZip::NullCompressor.instance
      @closed = false
      @current_entry = nil
      @comment = nil
    end
    ...
    def put_next_entry(entry_name, comment = nil, extra = nil, compression_method = Entry::DEFLATED, level = RubyZip.default_compression)
    ...
    def copy_raw_entry(entry)
      ...
      entry.get_raw_input_stream do |is|
        ...
        ::RubyZip::Entry.read_local_entry(is)
    ...
    def init_next_entry(entry, level = RubyZip.default_compression)
      finalize_current_entry
      @entry_set << entry
      entry.write_local_entry(@output_stream)
    ...
    def get_compressor(entry, level)
      ...
      when Entry::DEFLATED then
        ::RubyZip::Deflater.new(@output_stream, level)
      when Entry::STORED then
        ::RubyZip::PassThruCompressor.new(@output_stream)
      else
